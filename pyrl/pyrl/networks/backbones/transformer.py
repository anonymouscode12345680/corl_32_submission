...
class TransformerFrame(ExtendedModule):
    def __init__(self, num_frames, backbone_cfg, transformer_cfg=None, mask_type='full', only_decompose_action=False,
                 num_obj_frames=0):
        """
        ...
        only_decompose_action: whether to use different networks to process IDENTICAL point clouds at a single frame, 
            each network outputs different parts of action space (for debugging purpose)
        ...
        self.only_decompose_action = only_decompose_action
    ...
    def forward(self, pcd, hand_pose, robot_state, obj_pose_info=None, return_pre_tf=False, **kwargs):
        ...
        if not self.only_decompose_action:
            ...
            cur_hand_xyz = hand_pose[:, 0, :3]
            cur_hand_quat = hand_pose[:, 0, 3:]
            cur_hand_rot = quaternion_to_matrix(cur_hand_quat)
            cur_xyz = pcd['xyz'] - cur_hand_xyz[:, None, :]
            cur_xyz = torch.einsum('bni,bij->bnj', cur_xyz, cur_hand_rot)
